    ...
    private void logCurrentSaga(Event event, ETopics topic) {
        ...
        switch (event.getStatus()) {
            case SUCCESS -> log.info("### CURRENT SAGA: {} | SUCCESS | NEXT TOPIC {} | {} ",
                    source, topic, sagaId);
            case ROLLBACK_PENDING -> log.info("### CURRENT SAGA: {} | SEND TO ROLLBACK CURRENT SERVICE | NEXT TOPIC {} | {} ",
                    source, topic, sagaId);
            case FAIL -> log.info("### CURRENT SAGA: {} | SEND TO ROLLBACK PREVIOUS SERVICE | NEXT TOPIC {} | {} ",
                    source, topic, sagaId);
        }
    }
